...
def processRequest(req):
    if req.get("result").get("action") == "yahooWeatherForecast":
        ...
        yql_query=yql_query+ " WHERE user_group_code = '" + role + "' AND appl_status_code = 12 AND NVL (agent_flg, 'Z') = '"+type_flag+"' AND (branch_code ='"+branch_name.strip()+"' OR BRANCH_NAME LIKE'%"+branch_name.strip().upper()+"%')"
